...
function cost = costfunction(runningcosts, terminalcosts, system, ...
    ...
    x = computeOpenloopSolution(system, N, T, t0, x0, u, ...
                                atol_ode_sim, rtol_ode_sim, type);
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x(k,:), u(:,k),(k-1)*T);
    end
    cost = cost+terminalcosts(t0+(N+1)*T, x(N+1,:), N*T);
end
...
function [c,ceq] = nonlinearconstraints(constraints, ...
    ...
    ceq = [];
    for k=1:N
        [cnew, ceqnew] = constraints(t0+k*T,x(k,:),u(:,k),(k-1)*T);
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
    [cnew, ceqnew] = terminalconstraints(t0+(N+1)*T,x(N+1,:),N*T);
    c = [c cnew];
    ceq = [ceq ceqnew];
end
