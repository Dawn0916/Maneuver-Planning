...
T             = 0.2;
tmeasure      = 0.0;
x1measure = [+35.000 +0.000 +10 +2.625]; %initial states of Car1
x2measure = [+20.050 +0.000 +100 +7.875];%initial states of Car2
% y_ref0=[2.625 7.875];
y_ref=[];
...
function cost = runningcosts1(t, x, u, kT)
    ...
    global Vv2;
    global x_safe;
%     [ttc,tiv,p1,p2,p]=ttc_tiv();
    y_ref=x(4);
    Vx_ref=x(1);
    delta_v=x(1)-Vv2(end,1);
    
    delta_y=x(4)-((Xv2(end,2)+Xv2(end,2)*kT));
    delta_x=x(3)-((Xv2(end,1)+Vv2(end,1)*kT));
    abs_delta_x=abs(delta_x);  %longitudinal distance between vehicle1 and vehicle2
    delta_y=x(4)-((Xv2(end,2)+Xv2(end,2)*kT)); 
    abs_delta_y=abs(delta_y);  %lateral distance between vehicle1 and vehicle2
    
    % Longitudinal maneuver: Speed generation
    if x(4)<w_lane  %ego vehicle is on the right of the other vehicle 
%     if (abs_delta_y>b)&&(delta_y<(-1)*b)  %ego vehicle is on the right of the other vehicle         
        if abs_delta_x<x_safe
        Vx_ref=Vv2(end,1);%DE
        Vx_ref1=Vx_ref
        else
            Vx_ref=x(1);%CS
        ...
        if x(4)<2*w_lane % ego vehicle and other vehicle are in the same lane
%         if abs_delta_y<b   % ego vehicle and other vehicle are in the same lane
            if abs_delta_x>x_safe 
                Vx_ref=x(1); %CS
                Vx_ref2=Vx_ref
            else
                if abs_delta_x>(1/3)*x_safe
                    if delta_x<0
                        if delta_v<=0
                            Vx_ref=x(1); %CS
                            Vx_ref3=Vx_ref
                    ...
                    if delta_x<0
                        Vx_ref=Vv2(end,1)-3;%DE
                        Vx_ref7=Vx_ref
                    else
                        Vx_ref=Vv2(end,1)+3;%AC
                        Vx_ref8=Vx_ref
                    end
                end
            end
        else %ego vehicle is on the left of the other vehicle
            Vx_ref=Vv2(end,1)+15;%AC;
            Vx_ref9=Vx_ref
        end
    end

    %lateral maneuver: Lane choosing by goal
    if delta_x<0 %ego vehicle is behind the other vehicle
        if abs_delta_x>(2*x_safe)%can stay in first lane
            y_ref=(1/2)*w_lane;
            y_ref1=y_ref
        else
            if abs_delta_x>(1.5*x_safe)%change to second lane
                y_ref=(3/2)*w_lane;
                y_ref2=y_ref
        ...
        if delta_x<(1.5*x_safe)
            y_ref=(5/2)*w_lane; %stay in third lane
            y_ref4=y_ref
        else
            if abs_delta_x<(2*x_safe)%change to second lane
                y_ref=(3/2)*w_lane;
                y_ref5=y_ref
    ...
    %cost
    cost = 1*u(1)^2 + 0.1*u(2)^2 + 100*(x(1) - Vx_ref)^2 + 10*(x(4)-y_ref)^2;
...
%         if abs_delta_y<b   % ego vehicle and other vehicle are in the same lane
    ...
    %cost
    cost =100*(x(1) - Vx_ref)^2 + 10*(x(4)-y_ref)^2;
